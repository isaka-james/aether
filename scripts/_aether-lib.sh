...
PORT="$(read_env AETHER_AGENT_PORT)"; PORT="${PORT:-8765}"
...
start_agent_detached() {
  ...
  ( cd "$AGENT_DIR" && exec nohup env AETHER_HOST_AGENT_TOKEN="$t" python3 agent.py >>"$LOG" 2>&1 ) &
  echo $! >"$PIDFILE"
}
...
start_agent_child() {
  local t; t="$(read_env AETHER_HOST_AGENT_TOKEN)"
  ( cd "$AGENT_DIR" && exec env AETHER_HOST_AGENT_TOKEN="$t" python3 agent.py >>"$LOG" 2>&1 ) &
  echo $! >"$PIDFILE"
}
